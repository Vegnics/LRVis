...
def log_gradients(model):
    ...
    for k in grads:
        grads[k] = sum(grads[k]) / len(grads[k])
...
def train_one_epoch(
    ...
    accum_steps=1, grad_clip=None,epoch=0,depoch=10
):
    model.to(device)
    model.train()
    ...
    f_extr_t = create_feature_extractor(tmodel, ["layer1","layer2","layer3","layer4"])
    wce = 1.0
    wkd = 0.0 #0.6
    if epoch>depoch:
        wkd = 0.0
    ...
    if epoch < 7:
        ...
        for p in model.bn_final.parameters():
            p.requires_grad = False
    else:
        for p in model.fc.parameters():
...
def main():
    ...
    """
    train_tf = transforms.Compose([transforms.RandomResizedCrop(256, scale=(0.5, 1.0)),
                                   transforms.RandomHorizontalFlip(),
    ...
    #scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[25,45,60,80,90], gamma=0.1)
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30,45,80,90,95], gamma=0.1)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        tr_loss, tr_top1, tr_top5 = train_one_epoch(
            model,model_teach, train_loader,val_loader, criterion,critdistill, optimizer, device, scaler=scaler, log_every=50, accum_steps=4, grad_clip=None,epoch=epoch+1,depoch=15)
        ...
        if va_top1 > best_top1:
            best_top1 = va_top1
            save_checkpoint(save_dir / "best_distill_partial_res22.pt", model, optimizer, epoch, best_top1)
            printLog(f"  saved best_distill_partial_res22.pt (top1={best_top1:.2f}%)",LOGNAME)
        """
